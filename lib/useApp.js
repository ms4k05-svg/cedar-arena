...
export function useApp() {
  ...
  const [myTeamReg, setMyTeamReg] = useState(null);
  ...
  const fetchMyTeam = useCallback(async (tournamentId, userId) => {
    if (!tournamentId || !userId) { setMyTeam(null); setMyTeamReg(null); return; }
    const { data: membership } = await supabase.from("team_members").select("team_id").eq("user_id", userId);
    const teamIds = (membership || []).map((m) => m.team_id);
    if (teamIds.length === 0) { setMyTeam(null); setMyTeamReg(null); return; }
    const { data: team } = await supabase.from("teams").select("*").eq("tournament_id", tournamentId).in("id", teamIds).maybeSingle();
    if (!team) { setMyTeam(null); setMyTeamReg(null); return; }
    const { data: members } = await supabase.from("team_members").select("user_id, profiles(username, player_id)").eq("team_id", team.id);
    setMyTeam({ ...team, members: members || [] });
    const { data: reg } = await supabase.from("registrations").select("*").eq("team_id", team.id).maybeSingle();
    setMyTeamReg(reg || null);
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    fetchMyTeam(tournament?.id, profile?.id);
  }, [tournament?.id, profile?.id, fetchMyReg, fetchMyTeam]);
  ...
  // find out immediately when a captain adds me to (or removes me from) a team
  useEffect(() => {
    if (!profile?.id) return;
    const channel = supabase
      .channel(`cedar-my-team-${profile.id}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "team_members", filter: `user_id=eq.${profile.id}` },
        () => { fetchMyTeam(tournamentRef.current?.id, profile.id); }
      )
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [supabase, profile?.id, fetchMyTeam]);
  ...
  async function createTournament(payload) {
    ...
    const minTeam = parseInt(payload.minTeamSize, 10);
    const maxTeam = parseInt(payload.maxTeamSize, 10);
    const { data: code, error } = await supabase.rpc("create_tournament", {
      ...
      p_format: payload.format || "solo",
      p_min_team_size: Number.isFinite(minTeam) ? minTeam : null,
      p_max_team_size: Number.isFinite(maxTeam) ? maxTeam : null,
      p_check_in_required: !!payload.checkInRequired,
      p_score_reporting: payload.scoreReporting || "admin",
      p_require_screenshot: !!payload.requireScreenshot,
      p_bo3_from: payload.bo3From || "none",
      p_map_pool: payload.mapPool && payload.mapPool.length > 0 ? payload.mapPool : null,
    });
  ...
  async function adminSetTeamStatus(teamId, status) {
    if (!tournament) return;
    const { data: code, error } = await supabase.rpc("admin_set_team_status", {
      p_tournament_id: tournament.id, p_team_id: teamId, p_status: status,
    });
    if (error) { console.error("admin_set_team_status failed", error); notify(tr("err_generic")); return; }
    if (code === "full") { notify(tr("t_slots_full")); return; }
    await fetchTournamentData();
  }

  async function adminRemoveTeam(teamId) {
    const { error } = await supabase.rpc("admin_remove_team", { p_team_id: teamId });
    if (error) { console.error("admin_remove_team failed", error); notify(tr("err_generic")); return; }
    await fetchTournamentData();
  }
  ...
  async function reportWinner(roundIdx, matchIdx, player) {
    if (!tournament) return;
    const { data: code, error } = await supabase.rpc("report_winner", {
      p_tournament_id: tournament.id,
    ...
    if (code === "need_screenshot") { notify(tr("t_need_screenshot")); return; }
    await fetchTournamentData();
  }

  async function playerReportWinner(roundIdx, matchIdx, player) {
    if (!tournament) return;
    const { data: code, error } = await supabase.rpc("player_report_winner", {
      p_tournament_id: tournament.id,
      p_round_idx: roundIdx,
      p_match_idx: matchIdx,
      p_winner: player,
    });
    if (error) { console.error("player_report_winner failed", error); notify(tr("err_generic")); return; }
    if (code === "need_screenshot") { notify(tr("t_need_screenshot")); return; }
    if (code === "not_allowed") { notify(tr("err_generic")); return; }
    await fetchTournamentData();
  }
  ...
  async function createTeam(name) {
    if (!profile) { setView("auth"); return null; }
    const { data: code, error } = await supabase.rpc("create_team", { p_tournament_id: tournament.id, p_name: name });
    if (error) { console.error("create_team failed", error); return tr("err_generic"); }
    if (code === "closed") { notify(tr("t_reg_closed")); await fetchTournamentData(); return null; }
    if (code === "format") return tr("err_generic");
    if (code === "name") return tr("err_t_name");
    if (code === "full") { notify(tr("t_full")); return null; }
    if (code === "already") { notify(tr("t_already")); return null; }
    if (code === "name_taken") return tr("err_team_name_taken");
    await fetchMyTeam(tournament.id, profile.id);
    await fetchTournamentData();
    notify(tr("t_registered"));
    return null;
  }

  async function addTeamMember(teamId, identifier) {
    const { data: code, error } = await supabase.rpc("add_team_member", { p_team_id: teamId, p_identifier: identifier });
    if (error) { console.error("add_team_member failed", error); return tr("err_generic"); }
    const map = {
      not_captain: "err_generic",
      no_user: "err_team_no_user",
      already_on_team: "err_team_already",
      team_full: "err_team_full",
    };
    if (code) return tr(map[code] || "err_generic");
    await fetchMyTeam(tournament.id, profile.id);
    return null;
  }
  ...
  async function leaveTeam(teamId) {
    ...
    if (error) { console.error("leave_team failed", error); notify(tr("err_generic")); return; }
    await fetchMyTeam(tournament?.id, profile?.id);
  }

  async function removeTeamMember(teamId, userId) {
    const { error } = await supabase.rpc("remove_team_member", { p_team_id: teamId, p_user_id: userId });
    if (error) { console.error("remove_team_member failed", error); notify(tr("err_generic")); return; }
    await fetchMyTeam(tournament?.id, profile?.id);
  }

  async function checkIn() {
    if (!tournament) return;
    const { error } = await supabase.rpc("check_in", { p_tournament_id: tournament.id });
    if (error) { console.error("check_in failed", error); notify(tr("err_generic")); return; }
    await fetchMyReg(tournament.id, profile?.id);
    await fetchMyTeam(tournament.id, profile?.id);
    notify(tr("t_checked_in"));
  }
  ...
  return {
    ...
    tournament, history, settings, myReg, users, games, myGameIds, myTeam, myTeamReg,
    view, setView,
    ...
    adminSetTeamStatus, adminRemoveTeam,
    reportWinner, playerReportWinner, undoResult, finishTournament, deletePastTournament, saveSettings,
    register, reportNoShow, setGameId,
    createTeam, addTeamMember, leaveTeam, removeTeamMember, checkIn,
    refresh,
  };
}
