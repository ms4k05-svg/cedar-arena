...
import { roundName, totalRoundsOf, validPassword, validEmail, validPhone } from "./helpers";
...
export function useApp() {
  ...
  }, [supabase, fetchProfile, fetchTournamentData]);
  ...
  async function signup({ email, password, phone, tag, username }) {
    ...
    const { data: check } = await supabase.rpc("check_signup_available", {
      p_username: name,
      p_tag_raw: tag,
    });
    if (check) {
      const map = {
        uname_len: "err_uname_len",
        uname_taken: "err_uname_taken",
        tag_invalid: "err_tag",
        tag_taken: "err_tag_exists",
      };
    ...
    const { data, error } = await supabase.auth.signUp({
      email: em,
      password,
      options: { data: { username: name, phone: phone.trim(), player_tag: tag } },
    });
    if (error) {
  ...
  async function createTournament(payload) {
    ...
    const { data: code } = await supabase.rpc("create_tournament", {
      p_name: payload.name,
  ...
  async function cancelTournament() {
    ...
    await supabase.rpc("cancel_tournament", { p_id: id });
    await fetchTournamentData();
  ...
  async function setRegStatus(userId, status) {
    ...
    const { data: code } = await supabase.rpc("admin_set_registration_status", {
  ...
  async function removeReg(userId) {
    if (!tournament) return;
    await supabase.rpc("admin_remove_registration", { p_tournament_id: tournament.id, p_user_id: userId });
    await fetchTournamentData();
  }

  async function startBracket() {
    if (!tournament) return;
    const { data: code } = await supabase.rpc("start_bracket", { p_id: tournament.id });
  ...
  async function reportWinner(roundIdx, matchIdx, player) {
    if (!tournament) return;
    await supabase.rpc("report_winner", {
      p_tournament_id: tournament.id,
  ...
  async function undoResult(roundIdx, matchIdx) {
    if (!tournament) return;
    await supabase.rpc("undo_result", { p_tournament_id: tournament.id, p_round_idx: roundIdx, p_match_idx: matchIdx });
    await fetchTournamentData();
    notify(tr("t_undone"));
  }
  ...
  async function finishTournament() {
    ...
    await supabase.rpc("finish_tournament", { p_id: tournament.id });
    await fetchTournamentData();
    notify(tr("t_archived"));
  }

  async function deletePastTournament(id) {
    await supabase.rpc("delete_past_tournament", { p_id: id });
    setHistory((h) => h.filter((t) => t.id !== id));
    notify(tr("t_deleted"));
  }

  async function saveSettings(next) {
    await supabase.rpc("save_settings", { p_whish: next.whishNumber, p_contact: next.contact });
  ...
  async function register() {
    ...
    const { data: code } = await supabase.rpc("register_for_tournament", { p_tournament_id: tournament.id });
    if (code === "closed") { notify(tr("t_reg_closed")); await fetchTournamentData(); return; }
    if (code === "already") { notify(tr("t_already")); return; }
    if (code === "full") { notify(tr("t_full")); return; }
  ...
  async function reportNoShow(roundIdx, matchIdx) {
    if (!tournament) return;
    await supabase.rpc("report_no_show", { p_tournament_id: tournament.id, p_round_idx: roundIdx, p_match_idx: matchIdx });
    await fetchTournamentData();
    notify(tr("t_noshow"));
  }
  ...
  return {
    ...
    tournament, history, settings, myReg, users,
    view, setView,
